...
public partial class MainWindow : Window
{
    ...
    {
        await _mainWindowManagement.SetUser(_login);
        await SetButtons();          
        await InitializeUi();        
    }
    ...
    public async void AddPatientButtonClick(object sender, RoutedEventArgs routedEventArgs)
    {
        ...
        bool? result = newPatientWindow.ShowDialog(); 
        if (result == true)
        {
            await _mainWindowManagement.SetUser(_login);
            await SetButtons();
    ...
    public async void VisitStudyDocumentsPage(object sender, RoutedEventArgs routedEventArgs)
    {
        TeamMember user = _mainWindowManagement.GetUser();
        Study study = _mainWindowManagement.GetStudy();
        StudyDocuments studyDocuments = new StudyDocuments();
        studyDocuments.Owner = this; 
        bool? result = studyDocuments.ShowDialog();
        if (result == true)
        {
            InitializeComponent();
        }
    ...
    public async void ChangeStudyTeam(object sender, RoutedEventArgs routedEventArgs)
    {
        TeamMember user = _mainWindowManagement.GetUser();
        Study study = _mainWindowManagement.GetStudy();
        StudyManagement studyManagement = new StudyManagement(user, study);
        studyManagement.Owner = this; 
        bool? result = studyManagement.ShowDialog(); 
        if (result == true)
        {
            await _mainWindowManagement.SetUser(_login);
            await SetButtons();
